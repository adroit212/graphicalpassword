...
if (isset($_POST['proceed'])) {
    ...
    if ($result != NULL) {
        if ($result['role'] == "admin") {
            ...
            $otp_token = $ses->generateUnique(4);
            $expire_date = date('Y-m-d H:i:s', strtotime('+3 minutes'));
            $ses->newOTP($otp_token, $expire_date, $uname);

            $user = $ses->getSingleUser($uname);
            $mobile = $user['mobile'];
            $message = "Your Sign In OTP is " . $otp_token . ", expires in 3 minutes.";

            $res = $ses->sendSMS($mobile, $message);
...
if (isset($_POST['gsignin'])) {
    ...
    $result2 = $ses->checkOTP($url_uname, $otp);
    ...
    if ($result != NULL) {
        ...
        $ses->deactivateOTP($otp);

        header("location:pages/index.php");
        }else{
            $t_re_enc = base64_encode($url_uname);
        echo "<script>alert('Incorrect OTP!'); window.location.href='index.php?un=$t_re_enc';</script>";
        }
    } else {
        $t_re_enc = base64_encode($url_uname);
        echo "<script>alert('Incorrect password!'); window.location.href='index.php?un=$t_re_enc';</script>";
